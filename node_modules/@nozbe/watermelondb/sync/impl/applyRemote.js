...
var prepareApplyAllRemoteChanges = function (db, recordsToApply, sendCreatedAsUpdated, log, conflictResolver) {
  return (0, _rambdax.piped)(recordsToApply, (0, _rambdax.map)(function (records, tableName) {
    return prepareApplyRemoteChangesToCollection(db.get(tableName), records, sendCreatedAsUpdated, log, conflictResolver);
  }), _rambdax.values, _fp.unnest);
}; // See _unsafeBatchPerCollection - temporary fix


var unsafeBatchesWithRecordsToApply = function (db, recordsToApply, sendCreatedAsUpdated, log, conflictResolver) {
  return (0, _rambdax.piped)(recordsToApply, (0, _rambdax.map)(function (records, tableName) {
    return (0, _rambdax.piped)(prepareApplyRemoteChangesToCollection(db.collections.get(tableName), records, sendCreatedAsUpdated, log, conflictResolver), (0, _rambdax.splitEvery)(5000), (0, _rambdax.map)(function (recordBatch) {
    ...
    return new Promise(function ($return, $error) {
      ...
      return Promise.resolve(getAllRecordsToApply(db, remoteChanges)).then(function ($await_2) {
        try {
          ...
          return Promise.resolve(Promise.all([destroyAllDeletedRecords(db, recordsToApply)].concat((0, _toConsumableArray2.default)(_unsafeBatchPerCollection ? unsafeBatchesWithRecordsToApply(db, recordsToApply, sendCreatedAsUpdated, log, conflictResolver) : [db.batch( // $FlowFixMe
          prepareApplyAllRemoteChanges(db, recordsToApply, sendCreatedAsUpdated, log, conflictResolver))])))).then(function () {
            try {
              return $return();
            } catch ($boundEx) {
              return $error($boundEx);

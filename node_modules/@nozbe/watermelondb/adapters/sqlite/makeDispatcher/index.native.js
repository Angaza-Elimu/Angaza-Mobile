...
var makeDispatcher = function (type, tag, dbName) {
  ...
  var methods = dispatcherMethods.map(function (methodName) {
    ...
    return [methodName, function (...args) {
      ...
      if (jsiDb) {
        try {
          var value = 'query' === methodName || 'count' === methodName ? jsiDb[methodName].apply(jsiDb, (0, _toConsumableArray2.default)(otherArgs).concat([[]])) // FIXME: temp workaround
          : jsiDb[methodName].apply(jsiDb, (0, _toConsumableArray2.default)(otherArgs));
          callback({
            value: value
          });
        } catch (error) {
      ...
      var returnValue = DatabaseBridge[name].apply(DatabaseBridge, [tag].concat((0, _toConsumableArray2.default)(otherArgs)));

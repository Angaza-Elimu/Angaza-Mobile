...
class Quiz extends React.Component {
    ...
    answerQuestion(option_id) {
        if (this.state.question_answered == false) {
            if (this.state.current_question.answer == option_id) {
                ...
                this.setState({ question_answered: true, highlight_option: option_id })
                DataService.answerQuestion(this.state.subject_id, this.state.subtopic_id, 1, option_id, question_id, quiz_id).then(response => {
                    this.setState({ question_answered: true })
                    console.log(response);
                })
            }
    ...
    nextQuestion() {
        if (this.state.question_answered == true) {

            this.setState({ question_answered: false, highlight_option: null })
            let next_question = this.state.current_index + 1;
            this.setState({ current_index: this.next_question, highlight_option: null, question_answered: false, current_question: this.state.questions[next_question]})
        } else {
    ...
    render() {
        ...
        let question = '<meta name="viewport" content="width=device-width, initial-scale=1"> <body><p>This is the question</p></body>'

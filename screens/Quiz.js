...
class Quiz extends React.Component {
    ...
    getQuestions(value) {
        ...
        DataService.retrieveQuiz(value, 'quizq_questions').then(response => {this.setState({ questions: response, current_question: response[this.state.current_index], loading: false })}, err => { this.setState({ loading: false }); ToastAndroid.showWithGravity("Network connection error", ToastAndroid.LONG, ToastAndroid.CENTER); })
    }
    componentDidMount() {
    ...
    answerQuestion(option_id) {
        if (this.state.question_answered == false) {
            if (this.state.current_question.answer == option_id) {
                ...
                this.setState({ question_answered: true, highlight_option: option_id });
                DataService.answerQuestion(this.state.subject_id, this.state.selectedsubTopic, 1, option_id, this.state.current_question.id, this.state.user_id, 0, 'question_answers').then(response => {this.setState({ question_answered: true });ToastAndroid.showWithGravity("Answer Wrong", ToastAndroid.SHORT, ToastAndroid.TOP);})
    ...
    render() {
        return (
            <LinearGradient colors={["#2479AD", "#16ADA2", "#01FF90"]} start={[0.4, 0.1]} style={styles.gradientContainer}>
                <SafeAreaView style={styles.internalContainer}>
                    ...
                    {this.state.checkAnswer == false ? <View style={styles.screenMain}>
                        <View style={styles.headerRow}>
                            <View style={styles.headerRow}>
                                <Picker selectedValue={this.state.selectedTopic} placeholder="Topics" style={{ margin: 20, width: 150 }} onValueChange={(value) => {this.setState({ selectedTopic: value }); this.getSubtopics(value);}}><Picker.Item label="Select Topic" value="0" key="1" />{this.state.topics.length > 0 ? this.state.topics.map((item, index) => {return (<Picker.Item label={item.topic_name} value={item.id} key={index} />)}) : <Picker.Item label="No topics loaded" value="0" key="1" />}</Picker>
                                <Picker selectedValue={this.state.selectedsubTopic} placeholder="Subtopic" style={{ margin: 20, width: 150 }} onValueChange={(value) => { this.setState({ selectedsubTopic: value }); this.getQuestions(value);}}><Picker.Item label="Select Subtopic" value="0" key="1" />{this.state.subtopics.length > 0 ? this.state.subtopics.map((item, index) => { return (<Picker.Item label={item.subtopic_name} value={item.id} key={index} />)}) : <Picker.Item label="No topics loaded" value="0" key="1" />}</Picker>
                            </View>
                        </View>
                        <ScrollView> {this.state.questions.length > 0 ? <View style={styles.webViewContainer}>
